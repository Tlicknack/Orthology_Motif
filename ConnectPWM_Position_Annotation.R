...
#This script will take in a table generated by getPWM_Scores_Paramecium.R

#INPUT:
  #table with the following name: pwm_distribution_*type_*filename/or/motifSeq*-*number*.csv
    #"Consensus_Motif", "POFF_Row", "Species", "Scaffold", "Strand", "Position", "PWM_Score"
  #Intergenic GFFs (generated by makeIntergenic_GFF.R)
    #"scaffold"      "start_position"        "end_position"  "5'+strand"     "5'-strand"     "3'+strand"     "3'-strand"
#OUTPUT
...
for(i in 1:nrow(pwm_table)){ 
  ...
  if(pwm_row$Scaffold %in% gff_tab$scaffold){ 
    ...
    if(is.na(firstrow$scaffold)){ 
      ...
      #If the PWM position is larger than the start of the intergenic, then it is intergenic
    ...
    #Keep distances to genes only if genes are present... distance is only measured when is.na() comes back FALSE
